...
public:
	...
	void AddGuiFunction(std::string_view name, std::function<void()> func)
	{
	...
	void RemoveGuiFunction(std::string_view name)
	{
		if (!HasGuiFunction(name)) return;
		m_functions.erase(name);
	}
	bool HasGuiFunction(std::string_view name) const { return m_functions.contains(name); }
